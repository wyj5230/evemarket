...
public class MarketApi
{
  ...
  public static String callSearchKeyWordApi(String keyword) {
    ...
    try {
      String urlString = "http://evemarketer.com/api/v1/types/search?q="+ URLEncoder.encode(keyword, "UTF-8") +"&language=zh&important_names=false";
    ...
    if (searchResultArray.length() == 0){
      ...
      return "白面鸮级,Error发生，关键词'"+ keyword +"'没有返回匹配项";
  ...
  public static String callDetailItemApi(String id){
    ...
    try {
      ...
      if (detailItem == null) {
        return "白面鸮级,Error发生，id:'"+ id +"'没有返回匹配项";
      }
    } catch (IOException e) {
      System.out.println("Call eve market API failed, reason: " + e.getMessage());
      return "白面鸮级,Error发生，id:'"+ id +"'没有返回匹配项";
